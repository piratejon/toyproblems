...
from (
  ...
  from (
    select
      a, _what, _who, shift
      , t window_start
      , lead(t) over (order by rowid) window_stop
      , age(lead(t) over (order by rowid), t) d
      , extract(epoch from age(lead(t) over (partition by shift order by rowid), t)) / 60 dur
    from (
      select
        a, t, _what, shift, rowid
        , first_value(_who) over (partition by shift order by rowid) _who
      from (
        select
          rowid
          , a
          ...
          , sum(case when b[8] = 'Guard' then 1 else 0 end) over (order by rowid) shift
        from (
          select
            rowid
            , a
