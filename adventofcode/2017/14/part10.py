...
class Knot:
    ...
    def hash(self, string, length=256):
        '''do all 64 rounds for a hash'''
        self.list = [_ for _ in range(length)]
        self.len = len(self.list)
        self.curpos = 0
        self.skip = 0
        llist = [ord(_) for _ in string] + [17, 31, 73, 47, 23]
        for i in range(64):
            for _ in llist:
                self.do_it(_)
        return ''.join('{:02x}'.format(_) for _ in self.finalize())
...
def main(filename):
    ...
    if filename:
        with open(filename, 'r') as fin:
            ropelen, llist = 256, fin.read().strip()
    else:
        ropelen, llist = 256, [ord(_) for _ in '']

    a_llist = [ord(_) for _ in llist]
    a_llist += [17, 31, 73, 47, 23]
    ...
    anotherknot = Knot(ropelen)
    print('round2', anotherknot.hash(llist))

    print(anotherknot.hash('flqrgnkx-0'))
